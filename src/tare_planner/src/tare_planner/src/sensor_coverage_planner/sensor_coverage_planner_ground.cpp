...
bool PlannerParameters::ReadParameters(ros::NodeHandle& nh)
{
  ...
  kStopAfterLocalCoverageComplete = misc_utils_ns::getParam<bool>(nh, "kStopAfterLocalCoverageComplete", false);
  ...
  kExplorationMaxArea = misc_utils_ns::getParam<double>(nh, "kExplorationMaxArea", 100.0); // Default 100 square meters
  kMaxExplorationDistance = misc_utils_ns::getParam<double>(nh, "kMaxExplorationDistance", 100.0); // Default 100 meters
...
void SensorCoveragePlanner3D::execute(const ros::TimerEvent&)
{
  ...
  if (keypose_cloud_update_)
  {
    ...
    // Check explored area
    double resolution = pd_.planning_env_->GetPlannerCloudResolution();
    double explored_area = pd_.planning_env_->GetCoveredPointNum() * resolution * resolution;
    if (explored_area > pp_.kExplorationMaxArea)
    {
      if (!exploration_finished_)
      {
        PrintExplorationStatus("Max exploration area reached, returning home", false);
      }
      exploration_finished_ = true;
    }

    // Check max exploration distance
    if (GetRobotToHomeDistance() > pp_.kMaxExplorationDistance)
    {
      if (!exploration_finished_)
      {
        PrintExplorationStatus("Max exploration distance reached, returning home", false);
      }
      exploration_finished_ = true;
    }

    if ((pd_.grid_world_->IsReturningHome() || (pp_.kStopAfterLocalCoverageComplete && pd_.local_coverage_planner_->IsLocalCoverageComplete())) &&
