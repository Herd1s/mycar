...
#define REMOTE_RC_OFFSET 992
#define REMOTE_TOGGLE_DUAL_VAL 992

#define REMOTE_TOGGLE_THRE_VAL_A 592
#define REMOTE_TOGGLE_THRE_VAL_B 1392
// 摇杆死区：原来为 120（针对 -1024..+1024），现在通道经偏移为 -500..+500
// 按比例缩放到约 60，避免静态抖动引起误动作
#define DEAD_AREA	60
...
void sbus_frame_parse(remoter_t *remoter, uint8_t *buf)
{
    ...
    remoter->rc.ch[9] = ((buf[13] >> 3 | buf[14] << 5) & 0x07FF);
		...
		if(remoter->rc.ch[6-1] < REMOTE_TOGGLE_DUAL_VAL)
		{
			remoter->toggle.swb = 0;
		}
		else if(remoter->rc.ch[6-1] >= REMOTE_TOGGLE_DUAL_VAL)
		{
			remoter->toggle.swb = 1;
		}
		
		if(remoter->rc.ch[5-1] < REMOTE_TOGGLE_THRE_VAL_A)
		{
			remoter->toggle.swa = 0;
		}
		else if(remoter->rc.ch[5-1] >= REMOTE_TOGGLE_THRE_VAL_A && remoter->rc.ch[5-1] <= REMOTE_TOGGLE_THRE_VAL_B)
		{
			remoter->toggle.swa = 1;
		}
		else if(remoter->rc.ch[5-1] >= REMOTE_TOGGLE_THRE_VAL_B)
		{
			remoter->toggle.swa = 2;
		}
		
		if(remoter->rc.ch[8-1] < REMOTE_TOGGLE_THRE_VAL_A)
		{
			remoter->toggle.swd = 0;
		}
		else if(remoter->rc.ch[8-1] >= REMOTE_TOGGLE_THRE_VAL_A && remoter->rc.ch[8-1] <= REMOTE_TOGGLE_THRE_VAL_B)
		{
			remoter->toggle.swd = 1;
		}
		else if(remoter->rc.ch[8-1] >= REMOTE_TOGGLE_THRE_VAL_B)
		{
			remoter->toggle.swd = 2;
		}
		
		if(remoter->rc.ch[7-1] < REMOTE_TOGGLE_DUAL_VAL)
		{
			remoter->toggle.swc = 0;
		}
		else if(remoter->rc.ch[7-1] >= REMOTE_TOGGLE_DUAL_VAL)
		{
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef * huart, uint16_t Size)
{
    ...
    if(huart->Instance == UART5)
    {
        if (Size <= BUFF_SIZE)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(&huart5, rx_buff, BUFF_SIZE*2); // 重新启动接收 DMA 以接收下一个数据包
            sbus_frame_parse(&remoter, rx_buff);
            
//			memset(rx_buff, 0, BUFF_SIZE);
        }
        else  // 接收数据长度超过 BUFF_SIZE，重启接收并清空缓冲区
        {   
            HAL_UARTEx_ReceiveToIdle_DMA(&huart5, rx_buff, BUFF_SIZE*2); // 重新启动接收 DMA
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef * huart)
{
    if(huart->Instance == UART5)
    {
        HAL_UARTEx_ReceiveToIdle_DMA(&huart5, rx_buff, BUFF_SIZE*2); // UART 出错，重新启动接收 DMA
        memset(rx_buff, 0, BUFF_SIZE);                               // 清空接收缓冲区
    }
}
